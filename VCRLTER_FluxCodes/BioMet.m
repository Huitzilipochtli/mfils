...
function [BioMetFile] = BioMet(StartDate, EndDate)

% Monthly Biomet Data for full month to be used in eddypro processing
% This 1 min raw values to be written into csv files with format for
% eddypro eg. BioMET_2016_03_01_00.csv
...
PARout = 'PPFDr_1_1_1,'; WD = 'WD_1_1_1\n'; StrDate ='TIMESTAMP_1,';
...
if (datenum(StartDate,'yyyy-mm-dd HH:MM') >= newinstr)
    ...
    HeaderInfo = [StrDate TimeStr Ta1 Ta2 Ta3 Ts1 Ts2 Ts3 Tw1 Tw2 RH1 RH2...
        NetRad LWin LWout SWin SWout PARin PARout Alb Rg WSrmy SH1 SH2 P...
        sal Tide WD];
    HeaderUnits = ['yyyy-mm-dd,' 'HH:MM,' 'C,' 'C,' 'C,' 'C,' 'C,' 'C,' 'C,'...
        'C,' '%%,' '%%,' 'W+1m-2,' 'W+1m-2,' 'W+1m-2,' 'W+1m-2,'...
        'W+1m-2,' 'µmol+1m-2s-1,' 'µmol+1m-2s-1,'...
        'none,' 'W+1m-2,' 'm+1s-1,' 'W+1m-2,' 'W+1m-2,' 'mm,' 'mScm-1,' 'm,'...
        'degrees\n'];
    clear DateStr TimeStr Ta1 Ta2 Ta3 Ts1 Ts2 Ts3 RH1 RH2 NetRad LWin...
     ...
     HeaderInfo = [StrDate TimeStr Ta1 Ta2 Ta3 Ts1 Ts2 Ts3 RH1 RH2 NetRad...
        LWin LWout SWin SWout PARin PARout Alb Rg WSrmy SH1 SH2 WD];
    HeaderUnits = ['yyyy-mm-dd,' 'HH:MM,' 'C,' 'C,' 'C,' 'C,' 'C,' 'C,' '%%,'...
        '%%,' 'W+1m-2,' 'W+1m-2,' 'W+1m-2,' 'W+1m-2,'...
        'W+1m-2,' 'µmol+1m-2s-1,' 'µmol+1m-2s-1,' ,...
        'none,' 'W+1m-2,' 'm+1s-1,' 'W+1m-2,' 'W+1m-2,'...
        'degrees\n'];
    clear DateStr TimeStr Ta1 Ta2 Ta3 Ts1 Ts2 Ts3 RH1 RH2 NetRad LWin...
        LWout Pa SWin SWout PARin PARout WD Alb WSrmy SH1 SH2 Rg
end
...
% %     Tmp = accumarray(subs, OYST2014, [], @nanmean);
    ...
    for i=1:length(CRFiles);
        ...
        WD = MetData(:,19); PARin = MetData(:,20);
        ...
        if (Date >= newinstr)
            ...
            MetData = [Ta1 Ta2 Ta3 Ts1 Ts2 Ts3 Tw1 Tw2 RH1 RH2 NetRad...
                LWin LWout SWin SWout PARin PARout Alb Rg WSrmy SH1 SH2 P...
                sal Tide WD];
        else
            MetData = [Ta1 Ta2 Ta3 Ts1 Ts2 Ts3 RH1 RH2 NetRad LWin...
                LWout SWin SWout PARin PARout Alb Rg WSrmy SH1 SH2 WD];
        end
    ...
    for iLine = 1:size(BioMetData, 1) % Loop through each time/value row
        ...
        if (Date >= newinstr)
            fprintf(FileID, [repmat('%6.3f,',1,10) repmat('%8.5f,',1,15) '%8.5f\n'],...
                BioMetData(iLine, 1:end)) ; % Print the data values
        else
            fprintf(FileID, [repmat('%6.3f,',1,8) repmat('%8.5f,',1,12) '%8.5f\n'],...
                BioMetData(iLine, 1:end)) ; % Print the data values
        end
    end
    fclose(FileID);
